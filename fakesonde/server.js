...
getNmeaFakeData =function(config){
  // Les data non utiles sont fictives
  // $GPGGA,084558.61,5150.723,N,00-12.787,E,1,08,0.9,549.1,M,46.9,M, , *7D
  // $GPRMC,084558.61,A,5150.723,N,00-12.787,W,000.0,054.7,070224,020.3,E*5C
  ...
  var latdir=(config.latitude>0)?"N":"S"
  var lat=Math.abs(config.latitude)+((Math.random()-0.5)*config.variation.plani)
  var deg = Math.floor(lat)
  var mindec = (lat - Math.floor(lat)) * 60
  lat = (("0"+deg).substr(-2))+''+("0"+mindec.toFixed(3)).substr(-6)

  var londir=(config.longitude>0)?"E":"W"
  var lon=Math.abs(config.longitude)+((Math.random()-0.5)*config.variation.plani)
  deg = Math.floor(lon)
  mindec = (lon - Math.floor(lon)) * 60
  lon = (("00"+deg).substr(-3))+''+("0"+mindec.toFixed(3)).substr(-6)
  ...
  for (var frame of config.data) {
    ...
    switch(frame){
    case "GGA":
      newDataFrame="GPGGA,"+fix+","+lat+","+latdir+","+lon+","+londir+",1,08,0.9,"+alt+",M,46.9,M, , "
      break
    case "RMC":
      newDataFrame="GPRMC,"+fix+",A,"+lat+","+latdir+","+lon+","+londir+",000.0,054.7,"+fixDate+",020.3,E"
      break
    default :
...
writeDataToFile = function(data,fileName,mode="w"){
  const fullPath = config.dumpFolder+"/"+fileName
  ...
  fs.writeFileSync(fullPath,data,encoding,0o666,mode)
}
